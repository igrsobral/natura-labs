...
const TEST_DATA = {
  brands: {
    'Lifestyle Brand A': {
      ...
      categories: {
        'Fitness': { sales: [120, 150, 180, null, 210], average: 165, total: 660 },
        'Recovery': { sales: [90, 100, 85, 110, null], average: 96.25, total: 385 }
      }
    },
    'Fashion Brand A': {
      ...
      categories: {
        'Supplements': { sales: [300, 280, 310, 295, 330], average: 303, total: 1515 },
        'Gear': { sales: [50, null, 65, 70, 80], average: 66.25, total: 265 }
      }
    }
  },
  categories: {
    'Fitness': { totalSales: 660, average: 165, leadingBrand: 'Lifestyle Brand A', nullWeek: 'Week 4' },
    'Recovery': { totalSales: 385, average: 96.25, leadingBrand: 'Lifestyle Brand A', nullWeek: 'Week 5' },
    'Supplements': { totalSales: 1515, average: 303, leadingBrand: 'Fashion Brand A', nullWeek: null },
    'Gear': { totalSales: 265, average: 66.25, leadingBrand: 'Fashion Brand A', nullWeek: 'Week 2' }
  },
  weeklyTotals: {
...
export const generateContextualResponse = (query: string, analysis: QueryAnalysis): string => {
...
const generateBrandResponse = (query: string): string => {
  ...
  if (queryLower.includes('brand a trends') || queryLower.includes('show me brand a')) {
  ...
  const lifestyleBrand = queryLower.includes('lifestyle brand a') || queryLower.includes('lifestyle');
  const fashionBrand = queryLower.includes('fashion brand a') || queryLower.includes('fashion');
...
const generateComparisonResponse = (query: string): string => {
  ...
  if (queryLower.includes('category') || queryLower.includes('categories')) {
    const categories = Object.entries(TEST_DATA.categories)
      ...
      .map(([cat, data]) => `${cat}: ${data.totalSales} total (avg: ${data.average}, led by ${data.leadingBrand})`)
      .join(', ');
  ...
  const brands = Object.entries(TEST_DATA.brands)
    .sort(([,a], [,b]) => b.totalSales - a.totalSales)
    .map(([brand, data]) => `${brand}: ${data.totalSales} total (avg: ${data.averageSales.toFixed(1)})`)
    .join(', ');
...
const generateForecastResponse = (query: string): string => {
  const queryLower = query.toLowerCase();
  const timeframe = queryLower.includes('quarter') ? 'next quarter' : 
                   queryLower.includes('year') ? 'next year' : 'upcoming period';
...
const generatePerformanceResponse = (query: string): string => {
  const queryLower = query.toLowerCase();
  const isLookingForBest = queryLower.includes('best') || queryLower.includes('top') || queryLower.includes('highest');
...
export const generateFollowUpQuestions = (query: string, analysis: QueryAnalysis): string[] => {
