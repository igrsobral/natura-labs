...
export const useFocusManagement = (options: {
  ...
  const focusLast = useCallback(() => {
    if (containerRef.current) {
      const lastFocusable = FocusManager.getLastFocusableElement(containerRef.current);
      lastFocusable?.focus();
    }
  }, []);
...
export const useKeyboardNavigation = (handlers: Record<string, (event: KeyboardEvent) => void>) => {
  const handleKeyDown = useCallback((event: KeyboardEvent) => {
...
export const useRovingTabindex = (items: HTMLElement[], activeIndex: number) => {
  ...
  const handleKeyDown = useCallback((event: KeyboardEvent, onIndexChange: (newIndex: number) => void) => {
